...
def generate_metrics(path):
        ...
        test_y_pred = np.load('{}/test_predict_result_inverse.npy'.format(path))
        test_y_true = np.load('{}/test_label.npy'.format(path))
        test_y_true = np.reshape(test_y_true, (test_y_true.shape[0]))
        raw_test_y_true = np.load('{}/test_label_encoder.npy'.format(path))

        test_softmax_output = np.load('{}/test_softmax_feature.npy'.format(path))
