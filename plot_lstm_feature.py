...
def plot_tsne_for_different_motor_right_right(x, y, ax, motor):
    ...
    # ax.legend(loc="upper right")
    ax.set_title(
        "t-sne for lstm feature of motor {} with predict".format(motor))
...
def plot(path, train_motor, test_motor, ifPlot=False):
    ...
    # add
    test_predict_labels = np.load(
        '{}/test_predict_result_inverse.npy'.format(path))
    ...
    plot_tsne_for_different_motor_right(
        ...
        test_motor)
    plot_tsne_for_different_motor_right_right(
        tsne_result[sample_size:],
        np.reshape(test_predict_labels[test_sample], (sample_size)), axes[2],
        test_motor)
