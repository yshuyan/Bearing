...
# plot predict 
def plot_tsne_for_different_motor_right_right(x, y, ax, motor):
    ax.scatter(x[:, 0], x[:, 1], c=y, s=10)
    #ax.legend(loc="upper right")
    ax.set_title("t-sne for lstm feature of motor {} with predict".format(motor))
...
def plot(path, train_motor, test_motor, ifPlot=False):
    ...
    # add
    test_predict_labels = np.load('{}/test_predict_result_inverse.npy'.format(path))
    ...
    plot_tsne_for_different_motor_right_right(tsne_result[sample_size:], np.reshape(test_predict_labels[test_sample], (sample_size)), axes[2], test_motor)
    plt.savefig('{}/new_lstm_feature_tsne.jpg'.format(path))
    if ifPlot:
        plt.show()
# plot('Transfer_model/saved_model/2019_09_27_14_40_48_cnn_lstm_sliding_20_motor_train_0_test_3',0,3,True)
